import { parseAuthorizedKey } from "./public_key.js";
/** Parses one non-comment OpenSSH known_hosts line. */
export function parseKnownHost(line) {
    ...
    const { key, comment } = parseAuthorizedKey(`${keyType.value} ${encodedKey.value}${commentStart < line.length ? ` ${line.slice(commentStart)}` : ""}`);
...
export async function matchesKnownHost(entry, host) {
    let matched = false;
...
function requireWhitespace(text, start) {
...
function skipWhitespace(text, start) {
    let end = start;
    while (end < text.length && (text.charCodeAt(end) === 0x20 || text.charCodeAt(end) === 0x09))
        end++;
    return end;
}
async function matchesHostPattern(pattern, host) {
...
function parseHashedHost(pattern) {
...
function globMatches(pattern, value) {
    ...
    while (pattern.charCodeAt(patternIndex) === 0x2a)
        patternIndex++;
