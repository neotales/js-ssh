...
import { formatServiceAccept, formatUserAuthPublicKeySignatureData, formatUserAuthSuccess, parseServiceRequest, parseUserAuthPublicKeyRequest, } from "./auth.js";
import { connect, connectTcp, SSHClientError, SSHHostVerificationError, SSHRemoteExitError, } from "./client.js";
import { createAes128CtrHmacSha256Cipher } from "./cipher.js";
import { formatChannelClose, formatChannelData, formatChannelEof, formatChannelExtendedData, formatChannelOpenConfirmation, formatChannelRequestSuccess, formatChannelWindowAdjust, formatExitStatus, parseChannelClose, parseChannelData, parseChannelEof, parseChannelWindowAdjust, parseExecChannelRequest, parseSessionChannelOpen, parseSubsystemChannelRequest, } from "./connection.js";
import { computeCurve25519Sha256ExchangeHash, deriveX25519Secret, formatKexEcdhReply, formatKexInit, formatNewKeys, generateX25519KeyPair, parseKexEcdhInit, parseKexInit, parseNewKeys, } from "./kex.js";
import { generateEd25519KeyPair, signEd25519, verifyEd25519Signature } from "./keys.js";
import { formatPacket, readPacket } from "./packet.js";
import { formatSftpAttributes, formatSftpVersion, parseSftpInit, parseSftpStatRequest, readSftpPacket, } from "./protocol/sftp.js";
import { formatIgnore } from "./transport.js";
...
test("managed client runs a command with fragmented and coalesced stdout and stderr", async () => {
    ...
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
            ...
            await writeFragmentedPayloads(wire, formatChannelData({ recipientChannel: channel.local, data: new TextEncoder().encode("first ") }), formatChannelExtendedData({
                recipientChannel: channel.local,
                dataTypeCode: 1,
                data: new TextEncoder().encode("warn"),
            }), formatChannelData({ recipientChannel: channel.local, data: new TextEncoder().encode("second") }), formatExitStatus({ recipientChannel: channel.local, status: 0 }), formatChannelEof(channel.local), formatChannelClose(channel.local));
...
test("managed client returns nonzero command status as result data", async () => {
    ...
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
            ...
            await writeFragmentedPayloads(wire, formatExitStatus({ recipientChannel: channel.local, status: 23 }), formatChannelEof(channel.local), formatChannelClose(channel.local));
            strictEqual(parseChannelClose(await wire.readPayload()), channel.remote);
        },
    });
    const client = await connectClient(peer.transport, clientKey);
...
test("managed client multiplexes two commands with interleaved encrypted responses", async () => {
    ...
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
            ...
            await writeFragmentedPayloads(wire, formatExitStatus({ recipientChannel: second.senderChannel, status: 2 }), formatChannelEof(second.senderChannel), formatChannelClose(second.senderChannel), formatExitStatus({ recipientChannel: first.senderChannel, status: 1 }), formatChannelEof(first.senderChannel), formatChannelClose(first.senderChannel));
...
test("cancelling one multiplexed command does not close another", async () => {
    ...
    const peer = createPeer();
    let commandsStarted;
    const started = new Promise((resolve) => commandsStarted = resolve);
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
            const first = parseSessionChannelOpen(await wire.readPayload());
            const second = parseSessionChannelOpen(await wire.readPayload());
            ...
            await writeFragmentedPayloads(wire, formatChannelData({ recipientChannel: second.senderChannel, data: new TextEncoder().encode("still-running") }), formatExitStatus({ recipientChannel: second.senderChannel, status: 0 }), formatChannelEof(second.senderChannel), formatChannelClose(second.senderChannel));
            while (true) {
                const payload = await wire.readPayload();
                if (payload[0] === 93)
                    continue;
...
test("managed client bounds opening and closing channels to 64", async () => {
    ...
    const peer = createPeer();
    let openingsRead;
    const openings = new Promise((resolve) => openingsRead = resolve);
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
...
test("managed client opens an encrypted SFTP subsystem with fragmented channel data", async () => {
    ...
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
            ...
            await writeFragmentedPayloads(wire, formatChannelData({ recipientChannel: channel.local, data: version.subarray(0, 3) }), formatChannelData({ recipientChannel: channel.local, data: version.subarray(3) }));
            ...
            await writeFragmentedPayloads(wire, formatChannelData({ recipientChannel: channel.local, data: formatSftpAttributes(stat.id, { size: 4n }) }));
...
test("closing managed SFTP releases its lease for a later command", async () => {
    ...
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
            ...
            await wire.writePayload(formatChannelData({ recipientChannel: channel.local, data: formatSftpVersion({ version: 3, extensions: [] }) }));
            await closeSftpChannel(wire, channel);
            const command = await openExec(wire, "true");
            await writeFragmentedPayloads(wire, formatExitStatus({ recipientChannel: command.local, status: 0 }), formatChannelEof(command.local), formatChannelClose(command.local));
...
test("managed client runs a command while an SFTP subsystem is active", async () => {
    ...
    const server = serve(peer.server, hostKey, clientKey.publicKey, false, {
        onAuthenticated: async (wire) => {
            ...
            await writeFragmentedPayloads(wire, formatChannelData({
                recipientChannel: sftp.senderChannel,
                data: formatSftpVersion({ version: 3, extensions: [] }),
            }));
            await writeFragmentedPayloads(wire, formatExitStatus({ recipientChannel: command.senderChannel, status: 0 }), formatChannelEof(command.senderChannel), formatChannelClose(command.senderChannel));
            while (true) {
                const payload = await wire.readPayload();
                if (payload[0] === 93)
                    continue;
...
function hasLocalTcpCapability() {
...
function createDenoTcpEchoServer(deno) {
    ...
    let connections = 0;
    let closed = false;
    void listener.accept().then(async (connection) => {
        connections++;
        try {
...
async function createNodeTcpEchoServer(net) {
    let connections = 0;
...
function createPeer() {
    const clientToServer = new TransformStream(undefined, undefined, { highWaterMark: 1 });
    const serverToClient = new TransformStream(undefined, undefined, { highWaterMark: 1 });
    const transport = {
...
function trackedTransport(closeImmediately = false) {
...
async function connectClient(transport, credential) {
...
async function openExec(wire, command) {
...
async function openSftp(wire, maximumPacketSize = 32_768, initialWindowSize = 65_536) {
    const open = parseSessionChannelOpen(await wire.readPayload());
    const remote = 29;
    await wire.writePayload(formatChannelOpenConfirmation({
        recipientChannel: open.senderChannel,
...
async function readSftpChannelPacket(wire, remoteChannel) {
...
async function readSftpChannelData(wire, remoteChannel) {
    while (true) {
        const payload = await wire.readPayload();
        if (payload[0] === 93) {
            parseChannelWindowAdjust(payload);
...
async function closeSftpChannel(wire, channel) {
...
function requireSftpPacket(packet) {
    if (!packet)
        throw new Error("expected complete SFTP packet");
    return packet;
}
async function writeFragmentedPayloads(wire, ...payloads) {
...
async function expectWindowAdjustments(wire, channel, count) {
    for (let index = 0; index < count; index++) {
        const adjustment = parseChannelWindowAdjust(await wire.readPayload());
        strictEqual(adjustment.recipientChannel, channel);
    }
}
async function serve(server, hostKey, clientKey, corruptSignature = false, options = {}) {
    ...
    try {
        ...
        if (options.coalescePackets) {
            await wire.writeBytes(joinBytes(new TextEncoder().encode("preamble lines\r\nSSH-2.0-fake-server\r\n"), formatPacket(serverKexInit)));
        }
        else {
        ...
        await wire.writePayload(formatKexEcdhReply({ hostKey: hostKey.publicKey.marshal(), serverPublic: ephemeral.publicKey, signature }));
        parseNewKeys(await wire.readPayload());
        await wire.writePayload(formatNewKeys());
        wire.inboundCipher = await createAes128CtrHmacSha256Cipher(sharedSecret, exchangeHash, exchangeHash, "client-to-server");
        wire.outboundCipher = await createAes128CtrHmacSha256Cipher(sharedSecret, exchangeHash, exchangeHash, "server-to-client");
        ...
        strictEqual(await verifyEd25519Signature(clientKey, request.signature, formatUserAuthPublicKeySignatureData(exchangeHash, request)), true);
        if (options.coalescePackets) {
            await wire.writeBytes(joinBytes(await wire.outboundCipher.encrypt(formatUserAuthSuccess()), await wire.outboundCipher.encrypt(formatIgnore(new Uint8Array(34_950)))));
        }
        else {
