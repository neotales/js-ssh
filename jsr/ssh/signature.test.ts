...
import { parseSignature, SSHKeyError } from "./keys.ts";
import { SSHWriter } from "./primitives.ts";

function signatureWire(format: string, blob: Uint8Array): Uint8Array {
...
test("SSH signature parsing validates supported formats", () => {
  ...
  throws(
    () =>
      parseSignature(
        signatureWire("ecdsa-sha2-nistp384", new SSHWriter().writeMpint(0n).writeMpint(1n).toUint8Array()),
      ),
    SSHKeyError,
  );
  throws(
    () => parseSignature(signatureWire("ecdsa-sha2-nistp521", new SSHWriter().writeMpint(1n).toUint8Array())),
    SSHKeyError,
  );
});
