...
type NodeCrypto = {
  ...
  generateKeyPairSync(
    type: "x25519",
  ): { publicKey: { export(options: { format: "jwk" }): unknown }; privateKey: object };
  createPublicKey(options: { key: JsonWebKey; format: "jwk" }): object;
  diffieHellman(options: { privateKey: object; publicKey: object }): Uint8Array;
};
...
export function deriveX25519SecretSync(privateKey: object, peerPublicKey: Uint8Array): Uint8Array {
  assertX25519PublicKey(peerPublicKey);
...
export function computeCurve25519Sha256ExchangeHashSync(input: SSHCurve25519ExchangeHashInput): Uint8Array {
...
export function deriveKeyMaterialSync(
  sharedSecret: Uint8Array,
  exchangeHash: Uint8Array,
  sessionId: Uint8Array,
  label: SSHKeyMaterialLabel,
...
function encodeExchangeHashInput(input: SSHCurve25519ExchangeHashInput): Uint8Array {
...
function keyMaterialSeeds(
  sharedSecret: Uint8Array,
  exchangeHash: Uint8Array,
  sessionId: Uint8Array,
  label: SSHKeyMaterialLabel,
  length: number,
): { initialSeed: Uint8Array; nextSeed: (material: Uint8Array) => Uint8Array } {
...
function hashSha256Sync(input: Uint8Array): Uint8Array {
...
function isX25519Jwk(value: unknown): value is JsonWebKey & { x: string } {
  return typeof value === "object" && value !== null &&
    (value as { kty?: unknown }).kty === "OKP" && (value as { crv?: unknown }).crv === "X25519" &&
