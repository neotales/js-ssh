...
export function parseIdentification(line: string): SSHIdentification {
...
export function formatIdentification(identification: SSHIdentification): string {
 ...
 */
export function readIdentification(
  input: Uint8Array,
  maximumLineLength = 255,
): SSHIdentificationReadResult | undefined {
...
function hasIdentificationPrefix(input: Uint8Array, start: number, end: number): boolean {
  return end - start >= 4 && input[start] === 0x53 && input[start + 1] === 0x53 && input[start + 2] === 0x48 &&
    input[start + 3] === 0x2d;
}

function decodeAscii(input: Uint8Array, start: number, end: number): string {
  let value = "";
...
function assertPrintableAscii(value: string, name: string): void {
