...
export function parsePublicKey(wire: Uint8Array): SSHPublicKey {
  return new SSHPublicKey(wire);
}

/** Parses one ordinary OpenSSH authorized_keys public-key line without options or markers. */
export function parseAuthorizedKey(line: string | Uint8Array): AuthorizedKey {
  ...
  if (encodedStart === typeEnd) throw new SSHKeyError("invalid authorized-key line");
  const encodedEnd = readAuthorizedKeyFieldEnd(text, encodedStart);
  if (encodedEnd === encodedStart) throw new SSHKeyError("invalid authorized-key line");
...
export function formatAuthorizedKey(key: SSHPublicKey, comment = ""): string {
...
export async function fingerprintSHA256(key: SSHPublicKey): Promise<string> {
...
function decodeKeyType(bytes: Uint8Array): string {
...
function decodeBase64(encoded: string): Uint8Array {
  ...
  try {
    ...
    if (error instanceof SSHKeyError) throw error;
...
function readAuthorizedKeyFieldEnd(text: string, start: number): number {
  let index = start;
...
function isCanonicalBase64(encoded: string): boolean {
  ...
  if (padding > 2 || (padding > 0 && encoded.length - padding < 2)) return false;
  ...
  for (let index = 0; index < contentLength; index++) {
    ...
    if (!isLetter && !isDigit && code !== 0x2b && code !== 0x2f) return false;
  }
...
function removeBase64Padding(encoded: string): string {
