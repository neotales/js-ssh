...
combined output is bounded to 1 MiB by default and can be changed with `maximumOutputBytes`. Commands and
managed SFTP v3 subsystems can coexist through a private multiplexer with at most 64 opening, active, or
closing channels. Aborting a command closes and rejects only that channel; closing the parent terminates all
children.
...
`SSHClientError`. This client is intentionally limited: it has no interactive sessions, server API,
rekeying (post-authentication `KEXINIT` terminates the connection), or algorithm negotiation policy beyond
the one fixed modern profile. `close()`
