import { SSHParseError, SSHReader, SSHWriter } from "../primitives.ts";
...
Deno.test("SSHReader and SSHWriter roundtrip primitive values", () => {
  const bytes = new SSHWriter()
    .writeByte(42)
    .writeBoolean(false)
    .writeBoolean(true)
    .writeUint32(0xffff_ffff)
    .writeUint64(0xffff_ffff_ffff_ffffn)
...
Deno.test("SSH mpint uses canonical two's-complement encoding", () => {
  const vectors: Array<[bigint, number[]]> = [
    [0n, []],
    [127n, [0x7f]],
    [128n, [0, 0x80]],
    [0x9a37_8f9b_2e33_2a7n, [0x09, 0xa3, 0x78, 0xf9, 0xb2, 0xe3, 0x32, 0xa7]],
    [-1n, [0xff]],
    [-128n, [0x80]],
    [-129n, [0xff, 0x7f]],
  ];
...
Deno.test("SSHReader rejects noncanonical mpints", () => {
...
Deno.test("SSHReader enforces bounds, limits, and complete consumption", () => {
  ...
  assertThrows(
    () => new SSHReader(Uint8Array.of(0, 0, 0, 2, 1, 2), { maxStringLength: 1 }).readString(),
    SSHParseError,
  );
  assertThrows(() => new SSHReader(Uint8Array.of(1)).assertDone(), SSHParseError);
});

Deno.test("SSH name-lists reject empty and non-ASCII names", () => {
...
Deno.test("SSHWriter validates integer ranges", () => {
  ...
  assertThrows(() => new SSHWriter().writeUint64(0x1_0000_0000_0000_0000n), RangeError);
});
