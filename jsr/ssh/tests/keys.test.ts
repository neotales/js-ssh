...
function ecdsaWire(curve: string, pointLength: number): Uint8Array {
...
test("supported public-key blobs enforce algorithm-specific structure", () => {
  ...
  deepStrictEqual(
    parsePublicKey(
      new SSHWriter().writeString(new TextEncoder().encode("ssh-rsa")).writeMpint(65537n).writeMpint(3n).toUint8Array(),
    ).marshal(),
    new SSHWriter().writeString(new TextEncoder().encode("ssh-rsa")).writeMpint(65537n).writeMpint(3n).toUint8Array(),
  );

  throws(
    () => parsePublicKey(new SSHWriter().writeString(new TextEncoder().encode("ssh-ed25519")).toUint8Array()),
    SSHKeyError,
  );
  throws(
    () =>
  ...
  throws(
    () =>
      parsePublicKey(
        new SSHWriter().writeString(new TextEncoder().encode("ssh-rsa")).writeMpint(0n).writeMpint(3n).toUint8Array(),
      ),
    SSHKeyError,
  );
...
test("ssh-ed25519 signatures verify through native WebCrypto", async () => {
  ...
  const key = parsePublicKey(
    new SSHWriter().writeString(new TextEncoder().encode("ssh-ed25519")).writeString(publicKey).toUint8Array(),
  );
  const sshSignature = parseSignature(
