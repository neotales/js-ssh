...
import { type SFTPChannel, SFTPClient, SFTPStatusError } from "./sftp.ts";
...
test("SFTP client reassembles fragmented v3 packets for file and directory operations", async () => {
  ...
  await rejects(client.stat("/missing.txt"), (error: unknown) => error instanceof SFTPStatusError && error.code === 2);
...
test("SFTP pending request limit rejects locally and permits out-of-order responses", async () => {
  const requests: { id: number; path: string }[] = [];
  const peer = scriptedChannel((packet) => {
    if (packet[4] === 1) {
...
test("SFTP malformed input rejects requests and releases channel resources", async () => {
  let statReceived!: () => void;
  const received = new Promise<void>((resolve) => statReceived = resolve);
  const peer = scriptedChannel((packet) => {
    if (packet[4] === 1) {
      peer.send(formatSftpVersion({ version: 3, extensions: [] }));
    } else {
...
test("SFTP abort does not wait for an unresponsive CLOSE", async () => {
  ...
  let readDirReceived!: () => void;
  const reading = new Promise<void>((resolve) => readDirReceived = resolve);
  let closeRequests = 0;
  const peer = scriptedChannel((packet) => {
    switch (packet[4]) {
      case 1:
...
class FakeSftpServer {
  readonly files = new Map<string, Uint8Array>([["/remote.txt", Uint8Array.of(1, 2, 3, 4)]]);
  readonly channel: SFTPChannel;
  #controller!: ReadableStreamDefaultController<Uint8Array>;
  #nextHandle = 1;
  #handles = new Map<number, { path: string; directory: boolean; readDirectory: boolean }>();
  ...
  #receive(packet: Uint8Array): void {
    switch (packet[4]) {
      case 1: {
        ...
        this.#send(
          formatSftpVersion({ version: 3, extensions: [{ name: "vendor@example.test", data: Uint8Array.of(1) }] }),
        );
        return;
      }
      case 3: {
  ...
  #open(path: string, directory: boolean): number {
    const handle = this.#nextHandle++;
    this.#handles.set(handle, { path, directory, readDirectory: false });
    return handle;
  }

  #handle(bytes: Uint8Array): { path: string; directory: boolean; readDirectory: boolean } {
...
function scriptedChannel(onPacket: (packet: Uint8Array) => void | Promise<void>): {
  channel: SFTPChannel;
  send: (packet: Uint8Array) => void;
  closed: Promise<void>;
